...
def check_input_guardrails(user_message: str) -> InputGuardrailCheck:
    ...
    mini_llm = openAI.get("mini_llm")
    if mini_llm is None:
        raise ValueError(
            "Neither 'mini_llm' nor 'chat_llm' found in openAI dictionary. Please check src/services/llm.py"
        )
...
def create_simple_rag_agent(
    ...
    base_agent = create_agent(
        ...
        {"recursion_limit": 5}
    )  # ! Do not remove this. It is used to prevent infinite recursion.
    ...
    workflow.add_conditional_edges(
        "guardrail", should_continue, {"agent": "agent", "__end__": END}
    )
...
#     agent = create_simple_rag_agent(project_id="4fb3dfca-13d3-45ca-8cb2-5e7192c43904")
#     result = agent.invoke(
#         {"messages": [{"role": "user", "content": "Who is Rajat Sharma?"}]}
#     )
#     print(result)

...
"""
    ...
    if chat_history:
        ...
        if formatted_history:
            ...
            base_prompt += (
                "The following is the recent conversation history for context:\n\n"
            )
    ...
    def rag_search(
        ...
        try:
            ...
            response = prepare_prompt_and_invoke_llm(
                user_query=query, texts=texts, images=images, tables=tables
            )
...
def create_rag_agent(project_id: str, model: Optional[str] = "gpt-4o"):
    """
...
**Never answer without first querying the RAG tool. This ensures every response is grounded in project-specific context and documentation.**"""
    ...
    agent = create_agent(
        model=openAI["simple_agent_chat_llm"],
        tools=tools,
...
def create_web_search_agent(model: Optional[str] = "gpt-4o"):
    """
...
def create_supervisor_tools(project_id: str, model: Optional[str] = "gpt-4o"):
    """
    ...
    model = openAI["simple_agent_chat_llm"]
    rag_agent = create_rag_agent(project_id, model=model)
...
def create_supervisor_agent(
    ...
    workflow.add_conditional_edges(
        "guardrail", should_continue, {"supervisor": "supervisor", "__end__": END}
    )
...
#     content = "What does my document say about Rajat Sharma and search online to know more about him and give a short summary about him."
#     supervisor = create_supervisor_agent(
